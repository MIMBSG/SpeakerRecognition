...
export class SpeakerModelService {
  ...
  private recognition(mfcc: number[][]) {
    return new Promise(function (resolve,reject){
      ...
      let url: string = "http://localhost:8090/rest/user/recognition/48000/" + mfcc.toString();
      var scope = this;
  ...
  private register(mfcc: number[][], speakerModel: SpeakerModel){
    return new Promise(function (resolve,reject){
      let request: XMLHttpRequest = new XMLHttpRequest();
      let url: string = "http://localhost:8090/rest/user/register/48000/" + mfcc.toString() + "/" + speakerModel.name + "/" + speakerModel.lastName;
      var scope = this;
      request.onreadystatechange = function() {
        if (request.readyState === 4) {
            if (request.status === 200) {
        ...
        decode(reader.result).then( audioBuffer => {
          ...
          if(averageEnergy < 0.009){
            ...
            if(audioBuffer.duration < 3 || audioBuffer.duration > 8.99){
              resolve("Please speak longer than three but less than nine seconds.");
            }
            else
